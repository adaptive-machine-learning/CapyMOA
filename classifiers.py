...
# Library imports
from learners import MOAClassifier, MOARegressor, _get_moa_creation_CLI, _extract_moa_learner_CLI
...
class AdaptiveRandomForest(MOAClassifier):
    def __init__(self, schema=None, CLI=None, random_seed=1, base_learner=None, ensemble_size=100, 
                 max_features=0.6, lambda_param=6.0, # m_features_mode=None, m_features_per_tree_size=60,
        ...
        if CLI is None:
            self.base_learner = "(ARFHoeffdingTree -e 2000000 -g 50 -c 0.01)" if base_learner is None else _extract_moa_learner_CLI(base_learner)
        ...
        super().__init__(schema=schema, CLI=CLI, random_seed=random_seed, moa_learner=MOA_AdaptiveRandomForest())
...
class OnlineBagging(MOAClassifier):
    def __init__(self, schema=None, CLI=None, random_seed=1, base_learner=None, ensemble_size=100):
        ...
        # Initialize instance attributes with default values, if the CLI was not set. 
        if CLI is None:
            self.base_learner = "trees.HoeffdingTree" if base_learner is None else _extract_moa_learner_CLI(base_learner)
            self.ensemble_size = ensemble_size
            CLI = f"-l {self.base_learner} -s {self.ensemble_size}"

        super().__init__(schema=schema, CLI=CLI, random_seed=random_seed, moa_learner=MOA_OzaBag())
