...
from capymoa.evaluation.results import PrequentialResults, PrequentialClusteringResults
...
class ClusteringEvaluator:
    ...
    def _silhouette_score(self, clusterer: Clusterer, past_dps=None, future_dps=None):
        centers = np.array(clusterer._get_clusters_centers())
    ...
    def _ssq_score(self, clusterer: Clusterer, past_dps=None, future_dps=None):
        centers = np.array(clusterer._get_clusters_centers())
        ssq_scores = []
        # Assign each data point to the nearest cluster center
        if past_dps is not None and future_dps is not None:
        ...
        return ssq_scores
    def _bss_score(self, clusterer: Clusterer, past_dps=None, future_dps=None):
        centers = np.array(clusterer._get_clusters_centers())
        bss_scores = []
        if past_dps is not None and future_dps is not None:
            for _ in [past_dps, future_dps]:
    ...
    def update(self, clusterer: Clusterer, past_dps=None, future_dps=None):
        if self.clusterer_name is None:
            self.clusterer_name = str(clusterer)
        self.instances_seen += 1
        if self.instances_seen % self.update_interval == 0:
            self._update_measurements(clusterer, past_dps=past_dps, future_dps=future_dps)

    def _update_measurements(self, clusterer: Clusterer, past_dps=None, future_dps=None):
        ...
        if past_dps is not None and future_dps is not None:
            silhouette_scores = self._silhouette_score(clusterer, past_dps=past_dps, future_dps=future_dps)
            ssq_scores = self._ssq_score(clusterer, past_dps=past_dps, future_dps=future_dps)
            bss_scores = self._bss_score(clusterer, past_dps=past_dps, future_dps=future_dps)
