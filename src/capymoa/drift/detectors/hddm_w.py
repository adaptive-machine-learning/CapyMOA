...
class HDDMWeighted(MOADriftDetector):
    ...
    Reference:
    Frias-Blanco, Isvani, et al. "Online and non-parametric drift detection methods based on Hoeffding’s bounds."
    ...
    def __init__(self,
        ...
        if CLI is None:
            CLI = f'-d {drift_confidence} ' \
                  f'-w {warning_confidence} ' \
                  f'-m {lambda_} ' \
                  f'-t {test_type}'

        super().__init__(
            moa_detector=_HDDM_W_Test(),
            CLI=CLI
        )
    ...
    def add_element(self, element: float):
        if not isinstance(element, float):
            raise ValueError('Input element must be float type')
